...
//patch
#define QPNP_PON_KPDPWR_N_RESET_S2_CTL2_MASK  (0x80)
...
static int is_recovery_mode(void) {
	...
	for_each_available_child_of_node(pon->spmi->dev.of_node, pp) {
		...
		case PON_KPDPWR:
			...
			if (is_recovery_mode()) {
				cfg->support_reset = 0;
				rc = -22;
				printk(KERN_EMERG "[%s][LINE:%d] support_reset 0\n", __func__, __LINE__);
			}
			if (rc) {
				if (rc == -EINVAL) {
...
//patch
	...
	if (is_recovery_mode()) {
		u8 reg_value = 0;
		printk(KERN_EMERG "[%s][LINE:%d] yangli time2 disable\n", __func__, __LINE__);
		rc = spmi_ext_register_readl(pon->spmi->ctrl, pon->spmi->sid,
				QPNP_PON_KPDPWR_S2_CNTL2(pon),
				&reg_value, 1);
		printk("read before write QPNP_PON_KPDPWR_S2_CNTL2 rc = %d, reg_value = 0x%x\n", rc, reg_value);
		if (rc) {
			dev_err(&pon->spmi->dev,
				"Unable to read addr=%x, rc(%d) before write\n",
				QPNP_PON_KPDPWR_S2_CNTL2(pon), rc);
			//goto err_out;
		}

		rc = qpnp_pon_masked_write(pon, QPNP_PON_KPDPWR_S2_CNTL2(pon),
				QPNP_PON_KPDPWR_N_RESET_S2_CTL2_MASK, 0);
		printk("write QPNP_PON_KPDPWR_S2_CNTL2 rc = %d\n", rc);
		if (rc) {
			dev_err(&spmi->dev, "Unable to disable s2 timer rc:%d\n",
				rc);
			//goto err_out;
		}
		rc = spmi_ext_register_readl(pon->spmi->ctrl,pon->spmi->sid,
				QPNP_PON_KPDPWR_S2_CNTL2(pon), &reg_value,1);
		printk("read after write QPNP_PON_KPDPWR_S2_CNTL2 rc = %d, reg_value = 0x%x\n", rc, reg_value);
		if (rc) {
			dev_err(&pon->spmi->dev,
				"Unable to read addr=%x, rc(%d) after write\n",
				QPNP_PON_KPDPWR_S2_CNTL2(pon), rc);
			//goto err_out;
		}
	}
...
//patch end
	...
	if (is_recovery_mode()) {
		s3_debounce = 128;
		rc = 0;
		printk(KERN_EMERG "[%s][LINE:%d] s3_debounce 128\n", __func__, __LINE__);
	}
	...
	if(is_recovery_mode()){
		s3_src = "kpdpwr";
		rc = 0;
		printk(KERN_EMERG "[%s][LINE:%d] s3_src kpdpwr\n", __func__, __LINE__);
	}
	if (rc && rc != -EINVAL) {
		dev_err(&pon->spmi->dev, "Unable to read s3 timer rc: %d\n",
			rc);
	...
	if(is_recovery_mode()){
		pon->kpdpwr_dbc_enable = 1;
		printk(KERN_EMERG "[%s][LINE:%d] dbc_enable 1\n", __func__, __LINE__);
	}else{
